...
- **Which market a change belongs to is learned, not read from DraftKings' ids.** Changes usually carry only a selection id. The board lists each selection's market; after that, an add carries its market, and a line move's new id names the one it replaced. A change that can't be placed is skipped, not guessed (it only happens right after a reconnect, until a fresh board arrives).
